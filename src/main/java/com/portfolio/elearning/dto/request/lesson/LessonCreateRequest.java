...
@NoArgsConstructor
@AllArgsConstructor
@Schema(description = "Lesson creation request")
public class LessonCreateRequest {

    @NotNull(message = "Module ID is required")
    @Schema(description = "Module ID this lesson belongs to", example = "1")
    private Long moduleId;

    @NotBlank(message = "Lesson title is required")
    @Size(min = 3, max = 200, message = "Title must be between 3 and 200 characters")
    @Schema(description = "Lesson title", example = "Setting up the Development Environment")
    ...
    @Schema(description = "Lesson order in the module", example = "1")
    private Integer orderIndex;

    @Min(value = 1, message = "Duration must be at least 1 minute")
    @Schema(description = "Lesson duration in minutes", example = "15")
    private Integer durationMinutes;

    @Schema(description = "Enable preview for non-enrolled users", example = "false")
    private Boolean isPreviewEnabled = false;

    @Schema(description = "Is this lesson mandatory for completion", example = "true")
